...
layui.config({
    ...
    tab = layui.bodyTab({
        ...
        url: projectName+"/menu/getMenuByEmployeeNumber" //获取菜单json地址
    });
    ...
    //公告
    let notice;
    $.getJSON(projectName+'/notice/queryingSystemMessages', function (result) {
        setTimeout(function () {
            notice = result;
            ...
            for (let i = 0; i < notice.count; i++) {
                layer.open({
                    ...
                    yes: function (index, layero) {
                        $.getJSON(projectName+'/notice/noticeRead', {'nerId': notice.data[i].nerId});
                        layer.close(index);
